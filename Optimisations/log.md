...
## 2026-07-25 — Binary Search & Boundary Search (Algomonster)
...
## 2026-08-10 — Find All Anagrams (Algomonster)
tags: sliding-window, frequency-array, state-management, symmetric-updates, delta-tracking
mistake: (1) **Reset-on-success instinct** — after finding an anagram match (`wish_count == 0`), reset `wish_list` and `wish_count` back to initial values as if the window was empty, corrupting state for all subsequent slides. A match is just a snapshot; the sliding window state stays valid and only needs the normal incremental update. (2) **Asymmetric enter/exit updates** — decremented `wish_list[x]--` unconditionally when a character entered the window, but only incremented `wish_list[y]++` conditionally (`check.contains(y)`) when leaving. Non-check characters went negative on entry but were never restored on exit, causing permanent state drift. (3) **Wrong condition for `wish_count` changes** — used `check.contains(y)` (membership test) to decide if losing a character should increase `wish_count`. Correct signal is `wish_list[y - 'a'] >= 0` (deficit check): tells you the character was actively fulfilling a need, not surplus.
fix: (1) Removed all reset logic after match — just `res.push_back(tail)` and let the normal slide continue. (2) Made `wish_list[y - 'a']++` unconditional on exit, symmetric with the unconditional `wish_list[x]--` on entry. (3) Changed exit condition from `check.contains(y)` to `wish_list[y - 'a'] >= 0` to correctly track whether losing a character creates an unfulfilled wish.

## 2026-08-24 — Generate Subsets (CSES Book)
